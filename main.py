...
# Если для указания точности используется десятичная дробь (как в примере), то высчитываем число зннаков после запятой.
...
# Для рассчёта использована формула Лейбница
k = 1
s = 0
...
# числа из файла поместим в список, заполненный элементами типа "Строка".
with open('file.txt', 'r') as read_data:
    numbers = read_data.readlines()

# Проходимся по всему списку, и если элемент четный, то записываем его в файл.
...
print("Результат записан в файле 'file.txt'.")
